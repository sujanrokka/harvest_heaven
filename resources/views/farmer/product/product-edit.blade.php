        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div
                ...
                <div class="w-full space-y-5">
                    <form action="{{ route('farmer.product.update', $product) }}" method="POST"
                        enctype="multipart/form-data" class="w-full">
                        @csrf
                        @method('PUT') <!-- Adding this line to spoof PUT request -->
                        ...
                        <div class="mb-4">
                            ...
                            <textarea id="desc"
                                ...
                                name="desc" rows="3" required>{{ $product->desc }}</textarea>
                            @error('desc')
                                <ul class="text-sm text-red-600 space-y-1 mt-1">
                                    <li>{{ $message }}</li>
                                </ul>
                        ...
                        <div class="flex items-center justify-end mt-4">
                            <button type="submit"
                                class="py-2 px-4 bg-indigo-600 hover:bg-indigo-700 text-white rounded-md shadow-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2">Update
                                Product</button>
                        </div>
                    </form>
                </div>
